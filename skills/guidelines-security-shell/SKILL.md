...
- Reach for the allowed form first rather than discovering it from a block: workspace-relative targets, a specific PID over a pattern kill, the inner command over a wrapper. A hook confirm cannot be pre-satisfied in conversation — do gated steps while the user is present, or leave them undone and report them.
