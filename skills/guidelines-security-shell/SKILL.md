...
- Ordinary writes into another repository do not ask merely because that repository is outside the current workspace. When the user explicitly scopes one task across related repositories, that authorization covers all ordinary writes needed in those repositories for the task; never reconfirm per file, hunk, or tool call. Writes to non-repository external locations may still ask, and `guidelines-security-local` continues to refuse privacy, credential, authentication-store, and command-history paths. Reading ordinary paths anywhere does not ask. A relative path is read from wherever a preceding `cd` left the shell.
