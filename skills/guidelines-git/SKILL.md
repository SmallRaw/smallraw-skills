...
- Stage exact paths so routine work needs no approval at all. A hook confirm cannot be pre-satisfied in conversation — for unattended work, finish the gated steps while the user is present or leave them undone and report them. Never seek a way around the prompt.
