...
┌─────────────────────────────────────────────────────────────────┐
│  会话历史                                                        │
│  位置: ~/.claude/projects/{project-path-hash}/*.jsonl           │
│  性质: 只读数据源，项目所有对话的完整记录                           │
│  用途: 跨会话分析，发现长期模式，提取规则                           │
│  格式: 每行一个 JSON 对象 (type/message/timestamp/sessionId)     │
└─────────────────────────────────────────────────────────────────┘
```

**路径映射规则：**
- 项目路径 `/Users/foo/my-project` → 目录名 `-Users-foo-my-project`
- 即 `/` 替换为 `-`

**与规则层级的关系：**
- 会话历史是**规则的数据来源**，不是规则本身
- 通过分析历史 → 发现模式 → 生成规则建议 → 植入 Layer 1 (AGENTS.md)
- 这是 "规则是长出来的" 核心理念的数据基础
