...
| **问题信号** | "贡献者列表不完整" |
| **根本原因** | 忘记更新 |
| **规则** | 新贡献者的 PR 合并后，运行更新脚本，将头像添加到 README |
| **自动化** | `bun scripts/update-clawtributors.ts` |
| **来源** | CLAUDE.md 第 98-99 行 |
| **通用程度** | ⭐⭐⭐ 开源项目适用 |
...
| **问题信号** | "重复造轮子"、"风格不一致" |
| **根本原因** | 没有使用现有工具 |
| **规则** | 使用项目标准工具，如 CLI progress 用 `src/cli/progress.ts`，表格用 `src/terminal/table.ts` |
| **自动化** | 无，靠 review |
...
| **问题信号** | "颜色不一致"、"主题支持有问题" |
| **根本原因** | 硬编码颜色值 |
| **规则** | 使用共享 CLI palette（如 `src/terminal/palette.ts`），不硬编码颜色 |
| **自动化** | 无，靠 review |
...
| 项目 | 内容 |
|------|------|
| **问题信号** | "新 channel 缺少配置"、"UI 不完整" |
| **根本原因** | 遗漏了某些位置 |
| **规则** | 添加新 connection 时，更新所有 UI surface 和文档，添加 status + configuration forms |
| **自动化** | 无，靠检查清单 |
| **来源** | CLAUDE.md 第 138 行 |
| **通用程度** | ⭐⭐⭐ 有多 channel 的项目适用 |
...
| **规则** | 避免 `Type.Union`（无 anyOf/oneOf/allOf）；使用 `stringEnum`；避免 `format` 属性名 |
| **自动化** | 可在测试中检查 |
...
| **根本原因** | 链接格式不对 |
| **规则** | 内部链接用 root-relative，无 `.md`/`.mdx`；避免在标题中使用 em dash 和撇号 |
| **自动化** | 可用 broken-links 检查 |
| **来源** | commit `44a954b36` - add Mintlify link rules |
| **通用程度** | ⭐⭐⭐ 使用 Mintlify 的项目适用 |
...
### T3: 不要发送 streaming 到外部

| 项目 | 内容 |
|------|------|
| **问题信号** | "WhatsApp 收到了不完整的消息" |
| **根本原因** | streaming/partial 回复发到了外部渠道 |
| **规则** | 永远不要发送 streaming/partial 回复到外部消息表面（WhatsApp、Telegram）；只发最终回复 |
| **自动化** | 无，靠规则约束 |
| **来源** | commit `3fe68a051` - block partial replies on external chat surfaces |
| **通用程度** | ⭐⭐⭐ 有消息渠道的项目适用 |
...
| **问题信号** | "发布失败"、"OTP 过期了" |
| **根本原因** | 发布流程不对 |
| **规则** | 在 tmux session 中使用 1password skill 获取 OTP；发布后 kill tmux session |
| **自动化** | 1password skill |
...
| 知识规则 (K) | 8 | 文档、结构、指南 |
| 文档规则 (D) | 4 | 链接、格式、示例 |
| 工具规则 (T) | 3 | Schema、streaming |
| 发布规则 (R) | 4 | 版本、changelog、npm |
| **总计** | **56** | |
