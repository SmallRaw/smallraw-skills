...
- Reach for the allowed form first rather than discovering it from a block: an installed binary under `node_modules/.bin` or a package script instead of a one-off runner. In an unattended run, settle the installs the task needs before starting it.
