...
- Reach for the allowed form first rather than discovering it from a block: an installed binary under `node_modules/.bin` or a package script instead of a one-off runner. A hook confirm cannot be pre-satisfied in conversation — do the installs while the user is present, or leave them undone and report them.
